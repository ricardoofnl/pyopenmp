...
_fn = _capi.lazy("Object_GetMaterialText", ctypes.c_bool, [ctypes.c_void_p, ctypes.c_int, ctypes.POINTER(_capi.CAPIStringView), ctypes.POINTER(ctypes.c_int), ctypes.POINTER(_capi.CAPIStringView), ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_bool), ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int)])


def object_get_material_text(object, material_index):
    ...
    __ret = _fn(object, material_index, ctypes.byref(text), ctypes.byref(material_size), ctypes.byref(font_face), ctypes.byref(font_size), ctypes.byref(bold), ctypes.byref(font_color), ctypes.byref(background_color), ctypes.byref(text_alignment))
    if __ret:
